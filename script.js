...
(function () {
  ...
  const playerArt = $('.iplayer-art');
  const downloadBtn = $('#playerDownloadBtn');
  ...
  function loadTrack(i, autoplay) {
    ...
    if (downloadBtn) {
      ...
      downloadBtn.setAttribute('download', `${t.title} - IDEN`);
  ...
  function syncPlayingUI() {
    ...
    playerArt?.classList.toggle('is-playing', playing);
  ...
  downloadBtn?.addEventListener('click', e => {
    if (downloadBtn.classList.contains('is-disabled')) {
      e.preventDefault();
      notify('هنوز آهنگی برای دانلود منتشر نشده — به‌زودی 🎵');
    }
  });
  ...
  /* ---------- boot ---------- */
  if (downloadBtn && !hasTracks()) downloadBtn.classList.add('is-disabled');
  renderMusicSection();
  renderPlaylist();
})();
