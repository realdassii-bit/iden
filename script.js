...
(function () {
  ...
  /* ==========================================================
     🎵 TRACKS — این تنها جایی است که برای افزودن آهنگ لازم است
     ویرایش کنید. هر آبجکت یک قطعه است:

     title      نام آهنگ (الزامی)
     artist     نام هنرمند / زیرعنوان (اختیاری)
     cover      لینک مستقیم عکس کاور (jpg/png/webp)
     src        لینک مستقیم فایل صوتی (mp3/wav/ogg)
     comingSoon اگر true باشد، فقط به‌عنوان «به‌زودی» نمایش داده
                می‌شود و نیازی به cover/src واقعی نیست.

     لینک صوتی باید یک لینک مستقیم به فایل باشد (نه صفحه‌ی
     اسپاتیفای/ساندکلاود) — مثلاً از گیت‌هاب، دراپ‌باکس (با
     dl=1)، کتباکس یا هر هاست فایل مستقیم دیگر.
  ============================================================ */
  const TRACKS = [
    {
  ...
  /* ==========================================================================
     ...
     the popup — this is what makes it feel like real online streaming.
  ============================================================================ */
  ...
  const popupTrackName = $('#popupTrackName');
  const popupCoverImg = $('.player-mini-cover img');
  ...
  const dockPlayIcon = $('#dockPlayIcon');
  ...
  function trackAt(i) { return playableTracks[((i % playableTracks.length) + playableTracks.length) % playableTracks.length]; }
  ...
  function renderMusicSection() {
    ...
    if (musicFeaturedEl && featured && !featured.comingSoon) {
      musicFeaturedEl.innerHTML = `
        <div class="featured-track-cover" data-play-index="0">
          <img src="${featured.cover}" alt="${featured.title}" class="track-cover-img">
    ...
    if (musicGridEl) {
      musicGridEl.innerHTML = rest.map((t, i) => {
        ...
        return `
        <div class="music-card" data-play-index="${i + 1}">
  ...
  function renderPlaylist() {
    ...
    const head = playlistEl.querySelector('.iden-playlist-head');
    playlistEl.innerHTML = '';
    if (head) playlistEl.appendChild(head);
    else playlistEl.insertAdjacentHTML('beforeend', '<div class="iden-playlist-head"><span>PLAYLIST</span><span>SONIC UNIVERSE</span></div>');
    ...
    TRACKS.forEach((t, i) => {
      ...
      if (t.comingSoon || playableIdx === -1) {
        playlistEl.insertAdjacentHTML('beforeend', `
          <div class="iden-playlist-item disabled">
            <span class="playlist-num">${String(i + 1).padStart(2, '0')}</span>
            <span class="playlist-name">${t.title}</span>
            <span class="playlist-status">SOON</span>
          </div>`);
      } else {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'iden-playlist-item' + (playableIdx === current ? ' active' : '');
        btn.dataset.playableIndex = String(playableIdx);
        btn.innerHTML = `
          <span class="playlist-num">${String(i + 1).padStart(2, '0')}</span>
          <span class="playlist-name">${t.title}</span>
          <span class="playlist-status">PLAY</span>`;
  ...
  function highlightActivePlaylistItem() {
    $$('.iden-playlist-item[data-playable-index]').forEach(el => {
  ...
  function loadTrack(i, autoplay) {
    ...
    if (popupTrackName) popupTrackName.textContent = t.title;
  ...
  function play() {
    ...
    if (!audio.src || audio.src === window.location.href) {
      if (playableTracks.length) loadTrack(current, true);
      return;
    }
    audio.play().catch(err => console.warn('پخش امکان‌پذیر نشد:', err));
  }
  ...
  function goNext() {
    const next = shuffle ? Math.floor(Math.random() * playableTracks.length) : current + 1;
    loadTrack(next, true);
  }
  function goPrev() {
  ...
  audio?.addEventListener('ended', () => {
    ...
    if (playableTracks.length > 1) goNext();
    else syncPlayingUI();
  });
  audio?.addEventListener('error', () => { console.warn('فایل صوتی بارگذاری نشد.'); syncPlayingUI(); });
  ...
  progressBarEl?.addEventListener('click', e => seekFromEvent(progressBarEl, e));
  dockProgress?.addEventListener('click', e => seekFromEvent(dockProgress, e));
